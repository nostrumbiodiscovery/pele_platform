...
def test_covalent_docking_production():
    ...
    clusters = glob.glob(os.path.join(job2.pele_dir, "results", "clusters", "cluster*.pdb"))
    assert len(clusters) > 0


def test_ligands():
    """
    Runs covalent docking in test mode to check parametrization of unusual ligands, e.g. double bond (input1.yaml).
    """
    yaml_file = os.path.join(test_path, "input1.yaml")
