...
@pytest.mark.parametrize(("atom_dist", "expected_distances"),
                         [
                             ([], ""),  # no atom distances specified
                             (["A:672:N", "Z:900:N1"],  # 1 distance pair
                              '''
                             {"type": "com_distance",
                             "tag": "distance_A672N_Z900N1",
                              "selection_group_1": {"atoms": {"ids": ["A:672:_N__"]}},
                              "selection_group_2": {"atoms": {"ids": ["Z:900:_N1_"]}}},
                                '''
                              ),

                             (["A:672:N", "Z:900:N1", "A:783:N", "Z:900:C13"],  # 2 distance pairs
                              '''
                              {"type": "com_distance",
                               "tag": "distance_A672N_Z900N1",
                               "selection_group_1": {"atoms": {"ids": ["A:672:_N__"]}},
                               "selection_group_2": {"atoms": {"ids": ["Z:900:_N1_"]}}},
                              {"type": "com_distance",
                               "tag": "distance_A783N_Z900C13",
                               "selection_group_1": {"atoms": {"ids": ["A:783:_N__"]}},
                               "selection_group_2": {"atoms": {"ids": ["Z:900:_C13"]}}},
                               '''
                              ),
                         ])
def test_metric_builder_atom_dist(atom_dist, expected_distances):
    """
    Tests if the MetricBuilder correctly parses atom distances and assigned a meaningful metric name.

    Parameters
    ----------
    atom_dist : List[str]
        List of atom strings defining distances.
    expected_distances : str
        String expected to be generated by the MetricBuilder.
    """
    system = os.path.join(test_path, "site_finder", "complex.pdb")
    builder = MetricBuilder()
    distances = builder.distance_to_atom_json(system, atom_dist)

    assert distances.replace(" ", "").replace("\n", "") == expected_distances.replace(" ", "").replace("\n", "")
