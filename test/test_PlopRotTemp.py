...
import sys
import os
from test_config import parse_template, run_versions
...
#     old_sections, new_sections = run_versions(input_file)

#     old_names = old_sections[0][3]
#     new_names = new_sections[0][3]
#     num_atoms = len(old_names)
...
#     differences = False
#     for i, (old_section, new_section) in enumerate(zip(translated_old_NBND, NBND_new_ordered)):
...
def test_PlopRotTemp_BND_Param(input_file):
    ...
    new_sections, old_sections = run_versions(input_file)

    old_names = old_sections[0][3]
    new_names = new_sections[0][3]
    num_atoms = len(old_names)
  
    BND_old_sect = old_sections[2]
    BND_new_sect = new_sections[2]

    old_bond_1 = BND_old_sect[0]
    old_bond_2 = BND_old_sect[1]

    new_bond_1 = BND_new_sect[0]
    new_bond_2 = BND_new_sect[1]

    #"Hashing atom numbers to atom names"
    if(len(BND_old_sect)!=BND_new_sect):
      assert("Different number of bonds")
    for i, (old_atom1, old_atom2, new_atom1, new_atom2) in enumerate(zip(old_bond_1, old_bond_2, new_bond_1, new_bond_2)):
      old_bond_1[i] = old_names[int(old_atom1)-1].strip('_')
      old_bond_2[i] = old_names[int(old_atom2)-1].strip('_')
      new_bond_1[i] = new_names[int(new_atom1)-1].strip('_')
      new_bond_2[i] = new_names[int(new_atom2)-1].strip('_')
