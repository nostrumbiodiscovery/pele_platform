...
def get_distance_be(pointRef, path, resname):
	...
	return(bestDist, bestFileDistance, bestAtom, bestBE, bestFileBE, bestNormalize, bestNormFile, distBestNorm, beBestNorm)
...
def main (refFile = '', path="", atomCoords = '', resname = 'GRW'):
	if (atomCoords == ''):
		atomCoords = getAtomFromRef(refFile).get_coord()
	pathSearch = os.path.join(path, "*Core,*")
	...
	for file in configFiles:
		if os.path.isdir(os.path.join(file,"top_result")):
			...
			bestDist, bestFileDistance, bestAtom, bestBE, bestFileBE, bestNormalize, bestNormFile, distBestNorm, beBestNorm = get_distance_be(atomCoords, path, resname)
	...
	dataframe = {'File':files, 'BestFileDistance':bestFilesFoundDistance, 'Distance':dists, 'BestFileBE':bestFilesFoundBE, 'BE':BEs, 'BestFileNormalization': bestFilesFoundNorm, 'BestNormalization':norm, 'DistanceBestNormalization': distNormList, 'BEBestNormalization': beNormList}
	df = pd.DataFrame(dataframe, columns=['File','BestFileDistance','Distance','BestFileBE','BE','BestFileNormalization','BestNormalization','DistanceBestNormalization','BEBestNormalization'])
	df.to_csv(r'pandasOutput.csv', index = False, header=True)


if __name__ == "__main__":
	main(path="./", atomCoords=[1,1,1])
