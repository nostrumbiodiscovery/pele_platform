...
def _search_core_fragment_linker(ligand, ligand_core, result=0, check_simmetry=False):
    ...
    try:
        ...
        raise IndexError(
            "Make sure core from pdb and full fragment are the same. Be careful that either core and fragment have correctly define aromatic bonds. Also, all fragments must have different molecule name")

    # Sometimes substructure search messes up the symmetry. Check that!
    if check_simmetry:
