from Bio.PDB import PDBParser, PDBIO, Selection, NeighborSearch, rotmat, Vector
import numpy as np
import os
...
def prepare_structure(protein_file, ligand_pdb, chain):
    ...
    # remove additional protein chains and adjust water
    with open(protein_file, "r") as file:
        lines = file.readlines()
        for line in lines:
            if ((line.startswith("ATOM") or line.startswith("HETATM")) and line[21:22].strip() not in chain) or line.startswith("END") or line.startswith("TER") or line.startswith("CONECT"):
