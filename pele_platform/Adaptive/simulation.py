...
def run_adaptive(args):
    ...
    if env.adaptive_restart and not env.only_analysis:
        ...
        ################METAL CONSTRAINTS##################
        metal_constraints = mc.main(os.path.join(env.pele_dir, env.adap_ex_input.split(",")[0].strip().strip('"')), permissive=env.permissive_metal_constr, external=env.external_constraints)
