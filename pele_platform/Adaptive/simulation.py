...
def run_adaptive(env: pv.EnviroBuilder) -> pv.EnviroBuilder:
    ...
    if env.adaptive_restart and not env.only_analysis:
        ...
        env.logger.info("System: {}; Platform Functionality: {}\n\n".format(env.residue, env.package))
        
        if env.perturbation:
            syst = sp.SystemBuilder.build_system(env.system, env.mae_lig, env.residue, env.pele_dir)
        else:
            ...
            for input in env.input:
                ...
                if env.no_ppp:
                    ...
                    input_proc = os.path.basename(ppp.main(input_path, env.pele_dir, output_pdb=["" , ],
                                charge_terminals=env.charge_ter, no_gaps_ter=env.gaps_ter,
        ...
        elif env.full or env.randomize or env.ppi:
        ...
        if env.no_ppp:
            ...
            env.system, missing_residues, gaps, metals, env.constraints = ppp.main(syst.system, env.pele_dir, output_pdb=["" , ], charge_terminals=env.charge_ter, no_gaps_ter=env.gaps_ter, mid_chain_nonstd_residue=env.nonstandard, skip=env.skip_prep, back_constr=env.ca_constr, constrain_smiles=None, ligand_pdb=env.ligand_ref, ca_interval=env.ca_interval)

        # Metal constraints
        if not env.no_metal_constraints:
            metal_constraints, env.external_constraints = mc.main(env.external_constraints, os.path.join(env.pele_dir, env.adap_ex_input.split(",")[0].strip().strip('"')), syst.system, permissive=env.permissive_metal_constr, all_metals=env.constrain_all_metals, external=env.external_constraints, logger=env.logger)
        ...
        for res, __, _ in missing_residues:
            if res != env.residue and res not in env.skip_ligand_prep:
        ...
        water_obj = wt.WaterIncluder(input_waters, env.n_waters, 
            ...
            water_center=env.water_center, water_radius=env.water_radius,
        ...
        if env.atom_dist:
            env.metrics = metrics.distance_to_atom_json(env.system, env.atom_dist)
        else:
            env.metrics = ""
        if env.native:
            env.native = metrics.rsmd_to_json(env.native, env.chain)
        else:
            env.native = ""
