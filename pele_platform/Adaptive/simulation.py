...
def run_adaptive(args):
    ...
    if env.adaptive_restart and not env.only_analysis:
        ...
        ################METAL CONSTRAINTS##################
        metal_constraints, env.external_constraints = mc.main(args.external_constraints, os.path.join(env.pele_dir, env.adap_ex_input.split(",")[0].strip().strip('"')), syst.system, permissive=env.permissive_metal_constr, external=env.external_constraints)
