...
import pele_platform.Utilities.Helpers.yaml_parser as yl
from pele_platform.preprocess import pdb_fix
...
class Launcher():
    ...
    def launch(self):
        ...
        if self._args.system_fetch:
            self._args.system, self._args.chain, atom_map = pdb_fix.preprocess(
                self._args.system_fetch, self._args.residue,
                self._args.chain,
                rna=self._args.rna)
            #correct atom metric if atoms were changed
            if self._args.atom_dist and atom_map:
                self._args.atom_dist = [ atom_map[atom] for atom in self._args.atom_dist if atom_map]
...
def run_platform(input_yaml):
    ...
    arguments = yl.YamlParser(arguments.input_file)
