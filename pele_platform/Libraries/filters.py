...
def retrieve_files(path):
    """
    Find all SD files in user-defined directory.
    """
...
def is_toxic(mol):
    """
    Scan all filtered fragments for toxic/reactive substructures.
    """
    is_toxic = []
    ...
    for index, frag in enumerate(cs.toxic_frags):
        ...
        if frag == "[#16](=[#8])(-[#6])-[#6]":  # make sure thioketone is filtered out but not C-SO2-C
            if mol.HasSubstructMatch(pattern) and not mol.HasSubstructMatch(
                    Chem.MolFromSmarts("[#6]-[#16](-[#6])(=[#8])=[#8]")):
                is_toxic.append(mol)
        else:
