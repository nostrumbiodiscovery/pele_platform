# Inspired by the work of Sebastian Raschka, 2014
    ...
    with open(pdbfile, 'r') as pdb:
        ...
        for line in pdb:
            if line.startswith(include):
                ...
                try:
                    element_name = line[76:78].strip().upper()
                except IndexError:
                    element_name = '_missing_'

                if element_name not in ATOMIC_WEIGHTS:
                    atom_name = line[12:16].strip()
                    element_name = ''.join([c.upper() for c in atom_name
                                            if not c.isdigit()])
                try:
                    masses.append(ATOMIC_WEIGHTS[element_name])
                except KeyError:
                    raise KeyError("Element {} ".format(line[76:].strip().upper()) +
                                   "in input pdb not found " +
                                   "please change this ligand atom " +
                                   "element (last column of your pdb) " +
                                   "to an accepted element type")
