...
def set_water_control_file(env):
    ...
    if env.waters or env.n_waters:
        env.water_arg = hp.retrieve_all_waters(env.system) if env.waters == "all_waters" else env.waters # IDs of waters to perturb
        ...
        env.water = cs.WATER.format(all_waters, env.allow_empty_selectors, env.water_temp, env.water_trials, env.water_overlap, env.water_constr, env.waters).replace("'","")
